...
const CAREGIVER_TYPE_LABELS = { biological: "Biological or Adoptive Parent", foster: "Foster Parent", grandparent: "Grandparent / Extended Family", other: "Other Caregiver" };

function ChildProfileForm({ childCtx, onSaved, onCancel, onDeleted, showHeader = true }) {
...
const devLogSource = key => DEV_LOG_SOURCES.find(s => s.key === key) || DEV_LOG_SOURCES[0];
...
function DevLogSection({ activeChild, updateChild }) {
...
function TrainingScreen({ pop, account }) {
  const isAdmin = account?.role === "admin";
