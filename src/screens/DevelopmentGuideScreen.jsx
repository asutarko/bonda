...
function ActivitiesTab() {
  ...
  if (loading) return <p style={{ margin: 0, color: T.inkSoft, fontSize: 13 }}>Loading activities...</p>;
...
function TrainingTab() {
  ...
  if (loading) return <p style={{ margin: 0, color: T.inkSoft, fontSize: 13 }}>Loading...</p>;
...
export function DevelopmentGuideScreen() {
  const [tab, setTab] = useState("activities"); // activities | training
  ...
  return (
    <Page>
      ...
      <div style={{ display: "flex", background: T.border, borderRadius: T.r, padding: 3, gap: 3, marginBottom: 20 }}>
        {[["activities","Activities"],["training","Behaviour Training"]].map(([v, l]) => (
          <button key={v} onClick={() => setTab(v)} style={{ flex: 1, padding: "10px", borderRadius: 9, background: tab === v ? T.surface : "transparent", border: "none", fontWeight: 700, fontSize: 13, color: tab === v ? T.ink : T.inkMuted, cursor: "pointer", fontFamily: T.fontBody, boxShadow: tab === v ? T.shadow : "none", transition: "all 0.2s" }}>{l}</button>
        ))}
      </div>

      {tab === "activities" ? <ActivitiesTab /> : <TrainingTab />}
    </Page>
  );
}
